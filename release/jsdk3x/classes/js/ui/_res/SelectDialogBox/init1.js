...
function loadSelectData(selector,vValues,onFinished){
	selector.expand(vValues,{
		size: 100,
		onStart: function(){
			showStatus();
		},
		onLoading: function(values,start,count,seq,data){
			document.getElementById("status").innerHTML=para.oDspTexts.status.loadingData
							+" ("+((seq+1)*100)+"/"+(values.length-(start-seq*100))+") ...";
		},
...
function showStatus(){
	jsdk.PopupDialog.show(		
		"<table width=100% height=100%><tr valign=middle><td align=center height=100%><span id=\"status\"></span></td></tr></table>","",
		{
			owner: document.body,
