...
HTMLSelect.prototype.expand=function(values,para){
	var _this=this;
	if(!para) return this._expand(values);	
	if(!jsdk.isArray(values)) values=[values];
	if(values.length<1000) {
		this._expand(values); 
		if(para.onFinished) para.onFinished();
		return ;
	}
	para.onStart();
	jsdk.exec(function(values,start,count,seq,data){
		para.onLoading(values,start,count,seq,data);
		_this.expand(values.slice(start,start+count));
	},"while",{
		items: values,
		size: para.size,
		data: para.data,
...
HTMLSelect.prototype._expand=function(values){
	if(!jsdk.isArray(values)) values=[values];
