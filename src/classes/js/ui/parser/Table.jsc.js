 ...
 * @author: liu denggao
 * @created: 2012.4.1
 * @modified: 2012.4.5-2014.2.28
 * @log:
		2014.2.28 修复了根列模式下显示不正常的问题
...
with(_$class){
	...
	prototype.parse=function(){
		...
		if(data.arrangeMode=="row"){
			...
			htmlBody=this._parseColumnRange(data.data,true,true).join("");	//2014.2.27
		}	
	...
	/**
	 * @para vSize {array}: output parameter. 
	 */
	prototype._parseRowRange=function(vData,isRootRow,vSize){
		var html="",htmlTag="",htmlAttribs={},htmlBody="";
		var rows=[],htmlRows=[],maxRows=0,maxColumns=0,vSize1=[];
		rows=Global.is(vData,"Object")&&vData.data||vData;
		for(var i=0;i<rows.length;i++){
			var row=rows[i];
			if(Global.is(row,"Object")){
				if(row.arrangeMode=="row"){
					htmlRows.append(this._parseRowRange(row,isRootRow,vSize1));
					maxRows+=vSize1[0];
					if(vSize1[1]>maxColumns) maxColumns=vSize1[1];
				...
				maxRows++;
				htmlRows.push(this._parseRow([].fill("x",maxColumns),Global.is(vData,"Object")?vData.attribs:null,isRootRow));
			}	
		}
		if(vSize) { vSize[0]=maxRows;vSize[1]=maxColumns }
