{
	Application: {
		...
		"onInitEvent": function(){
			bindEvents([
				...
				[jsdk.dom("#btnMove")[0],{
					"onclick": events.ToolsBar["btnMove"]
				}],
				[jsdk.dom("#btnMoveAll")[0],{
					"onclick": events.ToolsBar["btnMoveAll"]
				}],
		...
		"onInitData": function(){
			...
			if(para.oPara.dspResult){
				...
				loadSelectData(thisform.F_Result,para.vValues);	//since v7.1
			}else if(!para.oPara.dspData&&!para.oPara.hasCategory){
				//thisform.F_View.expand(para.vValues);
				loadSelectData(thisform.F_View,para.vValues);	//since v7.1
			}
		}
	},
	SearchBar: {
				...
				if((para.oPara.hasChildItem||para.oPara.hasChildData)
					...
					if(para.oPara.hasChildItem){
						...
						if(jsdk.isArray(vValue)&&vValue.length==2){
							if(vValue[0]){	//is child data
								if(jsdk.isArray(vValue[0])){
									...
									loadSelectData(thisform.F_Data,vValue[0],function(){
										_this.onAfterChanged();
									});
								}else{
						...
						if(jsdk.isArray(vValue)){
							...
							loadSelectData(thisform.F_Data,vValue,function(){
								_this.onAfterChanged();
							});
						}else if(vValue){
	...
	ToolsBar: {
		"btnMove": function(){
			if(!para.oPara.dspResult) return;
			if(para.oPara.dspData){
				for(var i=0;i<thisform.F_Data.options.length;i++){
					var item=thisform.F_Data.options[i];
					if(item.selected){
						if(!para.oPara.isMultiple) thisform.F_Result.clear();
						thisform.F_Result.expand([[item.text,item.value]]);
					}
				}
			}else if(!para.oPara.hasChildItem&&!para.oPara.hasChildData){
				for(var i=0;i<thisform.F_View.options.length;i++){
					var item=thisform.F_View.options[i];
					if(item.selected){
						if(!para.oPara.isMultiple) thisform.F_Result.clear();
						thisform.F_Result.expand([[item.text,item.value]]);
					}
		...
		"btnMoveAll": function(){
			if(!para.oPara.dspResult) return;
			if(!para.oPara.isMultiple) return;
			if(para.oPara.dspData){
				var values=[];
				for(var i=0;i<thisform.F_Data.options.length;i++){
					var item=thisform.F_Data.options[i];
					//thisform.F_Result.expand([[item.text,item.value]]);
					values[i]=[item.text,item.value];
				}
				loadSelectData(thisform.F_Result,values);
			}else if(!para.oPara.hasChildItem&&!para.oPara.hasChildData){
				var values=[];
				for(var i=0;i<thisform.F_View.options.length;i++){
					var item=thisform.F_View.options[i];
