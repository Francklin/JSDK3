...
js.detect.shell.Browser=new function(){
	...
	var ua = navigator.userAgent.toLowerCase(),
		...
		UA = ua.match(/(opera|ie|firefox|chrome|maxthon|version|rv)[\s\/:]([\w\d\.]+)?.*?(safari|firefox|version[\s\/:]([\w\d\.]+)|$)/) || [null, 'unknown', 0],
		UA_Engine = ua.match(/(trident|gecko|webkit|presto)[\s\/:]([\w\d\.]+)?/) || [null, 'unknown', 0],
		mode = UA[1] == 'ie' && document.documentMode;
	this.name=(UA[1] == 'version' || UA[1] == 'rv') ? UA[3] : UA[1];
	...
	//-------
	if(UA[1] == 'rv' && Browser.Engine.name=="trident" ) Browser.name="ie";	//2014.6.18
